...
mod tests {
    ...
    #[test]
    fn ignores_semicolons_in_nested_block_comments() {
        let sql = "select /* outer /* ; */ outer */ 1; select 2";
        let expected_start = sql.rfind("select 2").unwrap();
        assert_eq!(
            range_at(sql, TextSize::from_usize(sql.len())),
            TextRange::new(
                TextSize::from_usize(expected_start),
                TextSize::from_usize(sql.len())
            )
        );
    }
