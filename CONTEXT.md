...
**源码范围（Loc）**:
由两个源码偏移量表示的半开区间 `[start, end)`。公开源码范围使用完整 SQL 的绝对坐标；语句内相对范围在对外返回前转换为源码范围。
_避免使用_: TextRange、闭区间

**ParseLoc**:
附着在部分原始解析树节点上的 PostgreSQL 兼容语义锚点，通常是引入该节点的语法 Token 的源码偏移量。`-1` 表示没有有效语义锚点。
_避免使用_: Location、源码范围

**源码位置（Position）**:
源码偏移量对应的从零开始的行号和列号。核心列号按 Unicode 标量计数；特定协议采用的其他列编码由 adapter 转换。
_避免使用_: 源码偏移量、显示列
